...
// Utility types to derive a camelCase "view" of the IDL, without modifying the
// underlying on-chain IDL naming conventions (which are typically snake_case).
type CamelCase<S extends string> = S extends `${infer Head}_${infer Tail}`
  ? `${Head}${Capitalize<CamelCase<Tail>>}`
  : S;

type CamelizeIdlNameValue<V> = V extends string ? CamelCase<V> : CamelizeIdl<V>;

type CamelizeIdl<V> = V extends readonly (infer U)[]
  ? readonly CamelizeIdl<U>[]
  : V extends object
  ? {
    [K in keyof V]: K extends "docs"
    ? V[K]
    : K extends "name" | "path"
    ? CamelizeIdlNameValue<V[K]>
    : K extends "relations"
    ? V[K] extends readonly (infer R)[]
    ? readonly (R extends string ? CamelCase<R> : R)[]
    : V[K]
    : CamelizeIdl<V[K]>;
  }
  : V;
...
*/
type RawDoxxCpmmDevnet = {
  address: "2sxKzYfW894TUYYRYNjeiWECj2Fwh9AMciv46cY78Y2Q",
  metadata: {
    name: "doxx_cp_swap",
    version: "0.2.0",
    spec: "0.1.0",
    description: "Doxx constant product AMM, supports Token2022 and without Openbook"
  },
  instructions: [
    {
    ...
    },
    {
      ...
      accounts: [
        ...
        },
        {
          ...
          writable: true,
          address: "Fu6zWJbBRN9jDuyyKcN9nd1YqH3oKMgdoFV6TP3UhnCZ"
        },
        {
...
export type DoxxCpmmDevnet = CamelizeIdl<RawDoxxCpmmDevnet>;