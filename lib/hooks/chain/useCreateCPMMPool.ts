  ...
  const createPool = useCallback(
    async (params: CreatePoolParams) => {
      ...
      try {
        ...
        if (!raydium) {
          setIsCreating(false);
          setCreateError(new Error("Raydium not initialized"));
          return undefined;
        }
        ...
        const mintA = await raydium.token.getTokenInfo(actualToken0Mint);
        const mintB = await raydium.token.getTokenInfo(actualToken1Mint);
        ...
        if (configAccount.disableCreatePool) {
          setCreateError(
            new Error("Pool creation is disabled for this fee tier"),
          );
          setIsCreating(false);
          onError(new Error("Pool creation is disabled for this fee tier"));
          return undefined;
        }

        const feeConfig: ApiCpmmConfigInfo = {
          id: ammConfig.toBase58(),
          index: configAccount.index,
          protocolFeeRate: configAccount.protocolFeeRate.toNumber(),
          tradeFeeRate: configAccount.tradeFeeRate.toNumber(),
          fundFeeRate: configAccount.fundFeeRate.toNumber(),
          createPoolFee: configAccount.createPoolFee.toString(),
          creatorFeeRate: configAccount.creatorFeeRate.toNumber(),
        };
        ...
        const { execute, extInfo } = await raydium.cpmm.createPool({
          ...
          mintA,
          mintB,
          mintAAmount: actualInitAmount0,
          mintBAmount: actualInitAmount1,
          startTime: openTime,
          feeConfig,
        ...
        console.log("🚀 ~ extInfo:", extInfo.address.lpMint);

        const { txId: sig } = await execute({ sendAndConfirm: true });
    ...
    [program, wallet?.publicKey, raydium, onSuccess, onError],
  );
